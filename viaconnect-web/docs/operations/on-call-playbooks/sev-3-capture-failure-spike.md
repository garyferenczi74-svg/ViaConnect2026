...
Status: RECONCILED PLAYBOOK, drafted 2026-06-01 (Prompt 171 Sections 8.2, 8.3, 4.1). Reconciled to the real formavision_quality_check_failed event and the body_scan_quality sub-scores per docs/operations/telemetry-architecture.md.

NOTE ON ALERTING: the alerting layer (PagerDuty or Opsgenie plus Sentry plus Better Uptime) is NOT deployed yet. The thresholds below are the SPEC to configure at launch; until then the capture-failure rate is watched manually via the formavision_ events in analytics_events and the Engineering Dashboard (docs/operations/dashboards/engineering-dashboard-spec.md).
...
- Threshold (spec to configure): formavision_quality_check_failed over formavision_capture_started breaches the Section 8.2 capture-failure threshold over a rolling window. Configure the exact value when the pager is wired.
...
- Event: formavision_quality_check_failed (formavision_ catalog). error_code is the COARSE reason category, never a measured value.
...
| Taxonomy bucket | formavision_quality_check_failed.error_code (coarse) | body_scan_quality sub-score | Blocking or advisory |
| --- | --- | --- | --- |
...
- A device or capture-mode specific issue (slice by device_model and capture_mode, both allowed metadata on formavision_capture_started).
...
1. Confirm the spike: formavision_quality_check_failed over formavision_capture_started above the threshold over the window.
