...
Status: RECONCILED PLAYBOOK, drafted 2026-06-01 (Prompt 171 Sections 8.2, 8.3). 171 calls this "FormaVision down"; the FormaVision rebrand does NOT exist in code. Reconciled to the REAL Body Scan path per docs/operations/telemetry-architecture.md.

NOTE ON ALERTING: the alerting layer (PagerDuty or Opsgenie plus Sentry plus Better Uptime) is NOT deployed yet. The thresholds below are the SPEC to configure at launch; until then this is watched manually via Vercel logs and the body_scan_ events in analytics_events.
...
- Threshold (spec to configure): processing_failed over processing_started breaches the Section 8.2 Body Scan failure threshold, or processing_completed drops sharply while capture_started holds. Configure the exact values when the pager is wired.
...
- A client-side regression in runScanAnalysis (a bad release breaking the MediaPipe or math pipeline) so processing_failed climbs with a client error_code.
...
1. Confirm the failing stage from the events: is the drop at processing_started (capture works, processing does not) or earlier? Compare processing_completed vs processing_failed and read processing_failed.error_code.
...
5. Check for a recent release: a client regression in runScanAnalysis shows as a spike in client-side processing_failed right after a deploy. Roll back the Vercel deployment if a release correlates.
