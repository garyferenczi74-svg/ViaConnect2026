...
- Tool: Metabase (BI over Supabase) for presentation, not deployed yet. Until it is, these read from the Supabase tables below plus the formavision_ consent events, surfaced at /admin/compliance and /admin/compliance/audit, which already exist and are RLS gated.
...
- Inclusivity waitlist: formavision_inclusivity_waitlist_joined event (by requested_capability) records demand for expanded scanning the current model does not serve.
...
| --- | --- | --- | --- |
| Consent acceptance rate | formavision_biometric_consent_accepted over formavision_biometric_consent_viewed; declines via formavision_biometric_consent_declined | analytics_events (formavision_) | RECONCILED |
...
| Inclusivity waitlist volume | formavision_inclusivity_waitlist_joined by requested_capability | analytics_events | RECONCILED |
