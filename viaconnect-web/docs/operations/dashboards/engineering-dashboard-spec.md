...
- Events: src/lib/body-tracker/scan-analytics.ts, the frozen body_scan_ catalog (24 events). The capture-flow funnel and the failure panels are built from these. The event prefix is body_scan_, NOT formavision_.
- Primary scan path: the client-side runScanAnalysis pipeline. It calls NO edge function and emits processing_started, then processing_completed with latency_seconds (or processing_failed with error_code) itself. So latency for the primary path is a client metric, not an edge-function metric.
...
- Quality data: canonical on body_photo_sessions (scan_quality_score, quality_issues). Sub-scores live in the supplementary body_scan_quality table (lighting, pose, clothing, bgClutter, cameraLevel, frameCoverage). The capture-failure event is quality_check_failed.
...
| Client processing latency (scan) | processing_completed latency_seconds (p50 to p99), split by success vs processing_failed | analytics_events (body_scan_) | RECONCILED |
| Capture-flow funnel | capture_started to capture_step_completed to processing_started to processing_completed to results_viewed, with capture_abandoned and capture_retake as drop and friction signals | analytics_events (body_scan_) | RECONCILED |
| Error rate / top error codes | processing_failed.error_code (coarse: no_photos, rate_limited, transient, and so on) plus safe-log error lines from the two edge functions | analytics_events + Vercel logs; Sentry once wired | RECONCILED (Sentry adds exception grouping) |
...
171 Section 4 specifies a capture-failure taxonomy and action triggers. Reconciled, the failure signal is the quality_check_failed event (body_scan_ catalog), whose error_code is the COARSE reason category, cross-referenced with the six body_scan_quality sub-scores for the underlying detail. The blocking-vs-advisory split is defined by aggregateQualityScores in src/lib/body-tracker/scan-quality.ts.

### 4.1 Failure taxonomy (reconciled to quality_check_failed.error_code plus body_scan_quality sub-scores)

| 171 taxonomy bucket | quality_check_failed.error_code (coarse) | body_scan_quality sub-score | Blocking or advisory |
| --- | --- | --- | --- |
...
Slice the failure taxonomy by capture_mode and device_model (both allowed metadata on capture_started). Do NOT slice by depth_sensor_type beyond confirming it is none for every scan; a non-none value would itself be an anomaly to investigate (it should not occur in Phase 1).
...
| --- | --- | --- |
| Capture-failure spike | quality_check_failed over capture_started exceeds the Section 8.2 threshold over a rolling window | Open the Sev 3 capture-failure playbook (docs/operations/on-call-playbooks/sev-3-capture-failure-spike.md); identify the dominant error_code; check whether a recent release changed the scan-quality thresholds (scan-constants.ts) or the capture UI |
...
| Processing-failure spike | processing_failed over processing_started exceeds the Section 8.2 elevated-error threshold | Open the Sev 2 elevated-errors playbook; check the body-scan-analyze breaker state and the Vision dependency (502 / 503 / 504 mix) |
