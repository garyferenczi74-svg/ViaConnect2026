...
names use the body_scan_ prefix (the formavision_ prefix does NOT exist), tier
slugs are free, gold, platinum, and platinum_family (there is no "Platinum Plus"),
...
4. Body Scan funnel health: capture to results conversion, quality check failure
   mix, processing success rate (real body_scan_ events, see funnel-optimization-log.md).
...
7. Engineering health: error rate, latency (client processing_completed
...
| First scan rate (signup to first capture_started) | capture_started | (estimate, confirm with Gary) | |
...
| Inclusivity waitlist joins | inclusivity_waitlist_joined | (estimate, confirm with Gary) | |
