// CANONICAL MOCKUP - Your Journey coaching layout (Prompt 208i Appendix A).
// This is the approved presentational source of truth. The TS page component
// (YourJourneyPageCoaching) reproduces this EXACTLY: same component set, DOM
// structure, inline style system, <style> block class names, and the 1100px /
// 960px breakpoints. Data is wired into the marked sample constants only;
// markup, classes, and styles are not changed. Reference only - not built.

import React, { useState, useEffect } from "react";
import {
  Search, Bell, Edit2, Target, Activity, Moon, Salad, Heart, Sparkles, Mail, Phone, RefreshCw,
...
const SW = 1.5;
const eyebrow = { textTransform: "uppercase", letterSpacing: 1.3, fontSize: 10.5, fontWeight: 700, color: C.muted };
...
function bandLabel(v) { return v >= 75 ? "Strong" : v >= 60 ? "Solid" : v >= 40 ? "Fair" : "Low"; }
function Delta({ v, unit }) { const up = v >= 0; return <span style={{ display: "inline-flex", alignItems: "center", gap: 3, fontSize: 11, fontWeight: 600, color: up ? C.teal : C.orange }}>{up ? <TrendingUp size={11} strokeWidth={SW} /> : <TrendingDown size={11} strokeWidth={SW} />}{up ? "+" : ""}{v}{unit || ""}</span>; }

function PlasmaRing({ value, color, size = 40 }) {
...
function GaugeCard({ value, label, color, hero }) {
  return (
...
function series(base, slope, n, seed) {
  const a = []; let s = seed;
...
const SLOPE = { sleep: -0.3, energy: 1.3, mood: 0.8, nutrition: 1.4, activity: 1.2, overall: 1.6, hydration: 1.3 };
const BASE = { sleep: 44, energy: 50, mood: 46, nutrition: 60, activity: 52, overall: 48, hydration: 55 };
function buildRange(n) { const o = {}; PILLARS.forEach((p, i) => { o[p.key] = series(BASE[p.key], SLOPE[p.key] * (n > 8 ? 0.7 : 1), n, (i + 1) * 97 + n); }); return o; }
const RANGE_DATA = { "1W": buildRange(7), "1M": buildRange(12), "1Y": buildRange(12) };
...
function StatBar({ icon: Icon, name, value, sub, pct, color }) {
  return (
...
function Sparkline({ data, color = C.teal, w = 92, h = 26 }) {
...
const VITALS = [["HRV", "72 ms", "+6", [60, 64, 62, 68, 66, 70, 72], C.teal], ["Resting HR", "58 bpm", "+3", [62, 61, 60, 59, 60, 58, 58], C.teal], ["Respiratory", "14.2", "+1", [15, 14.8, 14.5, 14.6, 14.3, 14.2, 14.2], C.teal], ["Blood Oxygen", "97 %", "+1", [96, 96, 97, 96, 97, 97, 97], C.teal], ["Hydration", "1.6 L", "+0.2", [1.2, 1.4, 1.3, 1.5, 1.4, 1.6, 1.6], "#38BDD8"]];
function Donut({ segments, top, bot, size = 118 }) {
...
const HUBS = [["CAQ", ClipboardList], ["Genetics", Dna], ["Labs", FlaskConical], ["Biology", HeartPulse], ["Nutrition", Salad], ["Supplements", Pill]];
...
export default function App() {
  useEffect(() => { const id = "vc-font-link"; if (!document.getElementById(id)) { const l = document.createElement("link"); l.id = id; l.rel = "stylesheet"; l.href = "https://fonts.googleapis.com/css2?family=Instrument+Sans:wght@400;500;600;700&display=swap"; document.head.appendChild(l); } }, []);
  return (
