...
| Kelsey gate (compliance) | TODO-AT-ARMING | Gary supplies/confirms sender identity and the physical postal address line; security-advisor reviews email content, sender, and unsubscribe. Facts found during this build that the review must resolve BEFORE arming: (1) the rendered emails currently contain NO unsubscribe link and NO physical postal address (CASL/CAN-SPAM requires both); (2) no unsubscribe endpoint exists yet (only the `practitioner_waitlist.unsubscribed` column, which the mailer honors, and a "you may unsubscribe at any time" sentence on `src/app/practitioners/page.tsx`); (3) legal entity string conflict: the email closing says "FarmCeutica Wellness LLC" while the 210f plan Global Constraints say "Farmceutica Wellness Ltd is the legal entity string on any customer-facing legal text". Gary confirms the correct string; nothing was changed here because fabricating legal identity is prohibited. |
...
1. Gates: every BUILT row above green on main; Kelsey gate cleared (content
   updated with Gary's confirmed legal entity string, physical address line,
   and a working unsubscribe path; security-advisor sign-off recorded).
