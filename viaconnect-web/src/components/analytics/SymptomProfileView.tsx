      ...
      <AnimatePresence>
        {open && (<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }} className="overflow-hidden">
          <div className="px-4 md:px-5 pb-5 pt-2 border-t border-white/5 space-y-5">
            ...
            {pattern.supplementProtocol?.length > 0 && (<div><p className="text-[10px] text-teal-400 uppercase tracking-wider font-semibold mb-2">FarmCeutica Protocol</p><div className="space-y-2">{pattern.supplementProtocol.map((s, i) => (<div key={i} className="rounded-lg bg-teal-400/[0.03] border border-teal-400/10 p-3"><p className="text-xs font-medium text-white/70">{s.product} — {s.dosage} ({s.timing})</p><p className="text-[11px] text-white/30 mt-0.5">{s.rationale}</p></div>))}</div><a href="/peptide-protocol" className="mt-3 min-h-[44px] inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-purple-400/10 border border-purple-400/25 text-purple-400 text-xs font-medium hover:bg-purple-400/15 transition-all"><FlaskConical className="w-4 h-4" strokeWidth={1.5} />View Your Peptide Protocol<ArrowRight className="w-3.5 h-3.5" strokeWidth={1.5} /></a></div>)}
