...
export function ConsumerDashboard() {
  ...
  const [checkinRaw, setCheckinRaw] = useState<Record<string, unknown> | null>(null);
  ...
  const handleCheckinScores = useCallback((rawState: Record<string, unknown>) => {
    setCheckinRaw((prev) => ({ ...(prev ?? {}), ...rawState, _ts: Date.now() }));
    setPreviewRaw(null);
  }, []);

  // Called on every slider change (live preview, no DB write)
  const handleSliderPreview = useCallback((sliderState: Record<string, unknown>) => {
