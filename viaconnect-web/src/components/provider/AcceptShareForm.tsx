...
// AcceptShareForm — shared provider entry point used by both
// /naturopath/accept-share and /practitioner/accept-share. The patient
// hands over an 8-character invite code (XXXX-XXXX). On submit we call
// the SECURITY DEFINER protocol_share_accept RPC, which validates the
// code, the recipient email (if scoped), and atomically claims the
// share by setting provider_id = auth.uid() + status = 'active'.
...
interface AcceptShareFormProps {
  /** Where to send the provider after a successful claim */
  redirectPath: string;
  /** Label shown in the header pill (e.g. "Naturopath access") */
  audienceLabel: string;
}

export function AcceptShareForm({
  redirectPath,
  audienceLabel,
}: AcceptShareFormProps) {
