...
import { useOptionalPractitioner } from '@/context/PractitionerContext';
import { RecommendPeptideButton } from '@/components/practitioner/RecommendPeptideButton';
...
export function PeptideCatalogCard({ peptide }: PeptideCatalogCardProps) {
  ...
  // Practitioner / naturopath portals get a "Recommend" button instead of
  // the consumer Share button. Returns null on the consumer side.
  const practitioner = useOptionalPractitioner();
  const isPractitionerMode = practitioner !== null;
  ...
  return (
    ...
    >
      ...
      {/* Actions — practitioner/naturopath gets Recommend, consumer gets Share */}
      <div className="mt-3 flex gap-2">
        {isPractitionerMode ? (
          <RecommendPeptideButton
            slug={peptide.id}
            name={peptide.name}
            productType="peptide"
            category={peptide.category}
            deliveryForms={peptide.dosingForms.map((d) => d.form)}
            variant="compact"
          />
        ) : (
