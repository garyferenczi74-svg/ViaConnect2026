...
export default function DistributionTargetToggle({ platform, enabled }: { platform: string; enabled: boolean }) {
  ...
  async function toggle() {
    ...
    setBusy(true);
    try {
      const res = await fetch(`/api/admin/soc2/distribution-targets/${encodeURIComponent(platform)}`, {
        method: 'PATCH',
