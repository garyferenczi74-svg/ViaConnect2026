    ...
    <>
      <div className="flex items-start justify-between gap-2">
        ...
        {tile.pending || tile.pendingLabel ? (
          <span className="rounded-full border border-white/15 bg-white/5 px-2 py-0.5 text-[10px] uppercase tracking-wide text-white/55">
            {tile.pendingLabel ?? 'Coming soon'}
          </span>
        ) : null}
      </div>
      <div className="mt-auto flex flex-col gap-1 pb-10 pr-14">
        <h3 className="text-[15px] font-semibold leading-tight text-white md:text-base">
          {tile.title}
        </h3>
        <p className="line-clamp-3 text-[12px] leading-relaxed text-white/[0.62] md:text-[13px]">
          {subtext}
        </p>
      </div>
      {!tile.pending ? (
        <div className="absolute bottom-4 right-4 inline-flex items-center gap-1 rounded-full border border-[#5B8DEF]/30 bg-[#2A4C9E]/[0.12] px-3 py-1.5 text-[12px] font-medium text-white backdrop-blur-md md:bottom-5 md:right-5">
          <span>Open</span>
    ...
    <div className="space-y-4" data-testid="peptide-education-bento">
      ...
      >
        {PEPTIDE_BENTO_MOBILE_ORDER.map((id: PeptideBentoTileId) => {
          ...
          return (
            <li key={tile.id} className="contents">
              {renderTile(tile, 'min-h-[140px] col-span-1')}
            </li>
          );
        })}
      </ul>
      ...
      >
        <p className="text-sm font-medium text-white">{STATEMENT_B_HEADING}</p>
        <p className="mt-1 text-xs leading-relaxed text-white/50">{STATEMENT_B_BODY}</p>
      </div>

      <p
        data-testid="peptide-statement-a-g36"
        className="rounded-xl border border-white/10 bg-[var(--card)]/60 p-3 text-xs leading-relaxed text-white/45"
      >
        {STATEMENT_A_G36}
      </p>
    </div>
