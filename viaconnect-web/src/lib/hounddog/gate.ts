...
export type GateVerdict = 'approved' | 'blocked' | 'escalated';
...
export function evaluateHoundDogGate(item: StagingCandidate): {
  ...
  // Bioavailability wrong range
  if (/\b\d+\s*[-–—to]+\s*\d+\s*x\b/i.test(text) && !/10x to 28x/i.test(text) && !/10-28x/i.test(text)) {
    // Soft: escalate if claimy range present but not locked string
