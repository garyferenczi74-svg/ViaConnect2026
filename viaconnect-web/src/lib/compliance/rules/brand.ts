...
export const BIOAVAILABILITY_RANGE: Rule<string> = {
  ...
  description: "Bioavailability copy is Maximum Bioavailability. Fold-number house ranges are retired.",
  evaluate: (text, ctx = defaultCtx()) => {
    if (typeof text !== "string" || text.length === 0) return [];
    const findings: Finding[] = [];
    // Flag any numeric fold range used as a house bioavailability claim.
    ...
    while ((m = re.exec(text)) !== null) {
      ...
      findings.push(
        baseFinding(
          ...
          "P0",
          `Finding: bioavailability fold range "${low}-${high}" is retired. Use Maximum Bioavailability.`,
          "ViaConnect Standing Rule §0.2",
          redactExcerpt(text, m.index, 80),
          {
            kind: "auto",
            summary: "Replace fold-number bioavailability claims with Maximum Bioavailability.",
            action: "REPLACE_RANGE:Maximum Bioavailability",
          },
          ctx,
        ),
      );
