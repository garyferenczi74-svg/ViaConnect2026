...
import {
  ...
  AlertTriangle, AlertOctagon, Info, Zap, ChevronDown, Plus,
} from "lucide-react";
...
const TABS = [
  { id: "daily", label: "Daily Schedule", short: "Schedule", icon: CalendarClock },
  { id: "recommended", label: "Recommended Supplements", short: "Recommended", icon: Sparkles },
  { id: "interactions", label: "Medical & Herbal", short: "Interactions", icon: ShieldAlert },
  { id: "practitioner", label: "Find a Practitioner", short: "Practitioner", icon: UserSearch },
  { id: "shop", label: "Browse & Build Protocol", short: "Shop", icon: ShoppingBag },
];
...
export default function SupplementsPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: "linear-gradient(180deg, #0F1520, #1A2744)" }}>
      <section className="relative rounded-2xl overflow-hidden mx-2 sm:mx-4 md:mx-8 my-4 md:my-6">
        ...
        <div className="relative z-10 p-5 md:p-8 border-b border-white/5">
          ...
          {/* 5-TAB BAR */}
          <div className="mt-6 overflow-x-auto scrollbar-hide -mx-2 px-2" style={{ WebkitOverflowScrolling: "touch" }}>
            <div className="flex gap-1 bg-white/[0.03] rounded-xl p-1 border border-white/5 min-w-max">
              {TABS.map((tab) => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className={`flex items-center justify-center gap-2 min-h-[44px] px-3 md:px-4 py-2.5 rounded-lg text-xs md:text-sm font-medium transition-all duration-200 whitespace-nowrap ${
                    activeTab === tab.id ? "bg-white/[0.08] text-white shadow-sm border border-white/10" : "text-white/30 hover:text-white/50 hover:bg-white/[0.02]"
                  }`}>
                  <tab.icon className={`w-4 h-4 flex-shrink-0 ${activeTab === tab.id ? "text-teal-400" : "text-white/20"}`} strokeWidth={1.5} />
        ...
        <div className="relative z-10">
          <AnimatePresence mode="wait">
            <motion.div key={activeTab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }}>
              ...
              {activeTab === "practitioner" && <PractitionerTab />}
  ...
  return (
    <div className="p-5 md:p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div><h3 className="text-base font-semibold text-white">Today&apos;s Schedule</h3><p className="text-xs text-white/30 mt-0.5">{taken}/{all.length} taken</p></div>
...
function RecommendedTab() {
  return (
    <div className="p-5 md:p-8 space-y-6">
      ...
      <div className="space-y-3"><h4 className="text-xs text-white/25 uppercase tracking-wider font-semibold">FarmCeutica Products For You</h4>{RECS.map((rec) => (
...
function InteractionsTab() {
  return (
    <div className="p-5 md:p-8 space-y-8">
      <div><div className="flex items-center gap-3 mb-4"><PIcon icon={Pill} color="#60A5FA" size="sm" /><div><h3 className="text-base font-semibold text-white">Current Medications</h3><p className="text-xs text-white/30">0 medications on file</p></div></div><div className="rounded-xl bg-white/[0.02] border border-white/5 p-5 text-center"><p className="text-sm text-white/30">No medications on file</p></div></div>
      <div><div className="flex items-center gap-3 mb-4"><PIcon icon={AlertTriangle} color="#FBBF24" size="sm" /><h3 className="text-base font-semibold text-white">Known Allergies &amp; Adverse Reactions</h3></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div className="rounded-xl bg-white/[0.02] border border-white/5 p-4"><p className="text-[10px] text-white/20 uppercase tracking-wider font-semibold mb-3">Allergies</p><p className="text-xs text-white/25">No known allergies</p></div><div className="rounded-xl bg-white/[0.02] border border-white/5 p-4"><p className="text-[10px] text-white/20 uppercase tracking-wider font-semibold mb-3">Adverse Reactions</p><p className="text-xs text-white/25">None reported</p></div></div></div>
      <div><div className="flex items-center gap-3 mb-4"><PIcon icon={ShieldAlert} color="#2DA5A0" size="sm" /><div><h3 className="text-base font-semibold text-white">Interaction Analysis</h3><p className="text-xs text-white/30">Real-time checking across all substances</p></div></div><div className="rounded-xl bg-teal-400/[0.03] border border-teal-400/10 p-5 text-center"><Check className="w-6 h-6 text-teal-400 mx-auto mb-2" strokeWidth={1.5} /><p className="text-sm text-teal-400/80 font-medium">No Interactions Found</p><p className="text-xs text-white/25 mt-1">Your current medications and supplements have been checked</p></div></div>
    </div>
  );
}

/* ═══ TAB 4: FIND A PRACTITIONER ═══ */
function PractitionerTab() {
  return (
    <div className="p-5 md:p-8 space-y-8">
      <div className="rounded-xl bg-orange-400/5 border border-orange-400/15 p-5 md:p-6">
        <div className="flex items-start gap-4">
          <PIcon icon={ShieldAlert} color="#B75E18" size="md" />
          <div><h3 className="text-base md:text-lg font-semibold text-orange-400 mb-2">Important: Consult Before Starting</h3><p className="text-sm text-white/50 leading-relaxed mb-3">Your supplement protocol has been generated by AI based on your Clinical Assessment Questionnaire results. We strongly recommend consulting with a healthcare practitioner or naturopath before starting any new supplement regimen.</p><p className="text-xs text-white/30 leading-relaxed">A qualified practitioner can review your protocol, verify AI recommendations against your complete medical history, provide personalized dosage adjustments, and monitor your progress over time.</p></div>
        </div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
        <div className="relative rounded-2xl overflow-hidden group cursor-pointer"><div className="absolute inset-0 bg-gradient-to-br from-teal-400/10 to-teal-400/[0.02]" /><div className="relative border border-teal-400/15 group-hover:border-teal-400/30 rounded-2xl p-6 md:p-8 transition-all duration-300 group-hover:shadow-[0_0_30px_rgba(45,165,160,0.08)]">
          <PIcon icon={Stethoscope} color="#2DA5A0" size="lg" />
          <h3 className="text-lg font-bold text-white mt-4 mb-2">Find a Practitioner</h3>
          <p className="text-sm text-white/40 leading-relaxed mb-6">Connect with licensed practitioners who specialize in integrative medicine and genomics-guided health optimization.</p>
          <div className="space-y-2 mb-6">{["Review your AI protocol", "Verify medication interactions", "Order specialized lab work", "Monitor your progress"].map(t => <div key={t} className="flex items-center gap-2"><Check className="w-3.5 h-3.5 text-teal-400/60 flex-shrink-0" strokeWidth={2} /><span className="text-xs text-white/35">{t}</span></div>)}</div>
          <a href="/messages" className="min-h-[48px] w-full flex items-center justify-center gap-2 rounded-xl bg-teal-400/15 border border-teal-400/30 text-teal-400 font-semibold text-sm group-hover:bg-teal-400/20 transition-all"><Stethoscope className="w-4 h-4" strokeWidth={1.5} /> Browse Practitioners <ArrowRight className="w-4 h-4" strokeWidth={1.5} /></a>
        </div></div>
        <div className="relative rounded-2xl overflow-hidden group cursor-pointer"><div className="absolute inset-0 bg-gradient-to-br from-emerald-400/10 to-emerald-400/[0.02]" /><div className="relative border border-emerald-400/15 group-hover:border-emerald-400/30 rounded-2xl p-6 md:p-8 transition-all duration-300 group-hover:shadow-[0_0_30px_rgba(52,211,153,0.08)]">
          <PIcon icon={Leaf} color="#34D399" size="lg" />
          <h3 className="text-lg font-bold text-white mt-4 mb-2">Find a Naturopath</h3>
          <p className="text-sm text-white/40 leading-relaxed mb-6">Connect with naturopathic doctors who combine traditional healing with modern genomics and herbal protocols.</p>
          <div className="space-y-2 mb-6">{["Herbal protocol guidance", "TCM / Ayurvedic integration", "Functional lab interpretation", "Holistic wellness planning"].map(t => <div key={t} className="flex items-center gap-2"><Check className="w-3.5 h-3.5 text-emerald-400/60 flex-shrink-0" strokeWidth={2} /><span className="text-xs text-white/35">{t}</span></div>)}</div>
          <a href="/messages" className="min-h-[48px] w-full flex items-center justify-center gap-2 rounded-xl bg-emerald-400/15 border border-emerald-400/30 text-emerald-400 font-semibold text-sm group-hover:bg-emerald-400/20 transition-all"><Leaf className="w-4 h-4" strokeWidth={1.5} /> Browse Naturopaths <ArrowRight className="w-4 h-4" strokeWidth={1.5} /></a>
        </div></div>
      </div>
    </div>
  );
