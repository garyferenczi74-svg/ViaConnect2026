...
export default function NaturopathError({ error, reset }: ErrorBoundaryProps) {
  useEffect(() => {
    console.error('[error-boundary][naturopath]', {
      name: error.name,
    ...
    <div className="min-h-screen flex items-center justify-center p-4 sm:p-8 bg-[#1A2744]">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-6 sm:p-8 text-center">
        ...
        <p className="text-sm sm:text-base text-gray-600 mb-6 sm:mb-8">
          We encountered an unexpected issue loading the naturopath portal. Please try again, and if the problem persists, refresh the page.
        </p>
        <button
          onClick={reset}
