...
import { Layers, Zap, Check, Save, Search, Info } from 'lucide-react';
...
export default function PractitionerMediaSourcesPage() {
  const [activeSources, setActiveSources] = useState<string[]>([]);
  const [categoryFilter, setCategoryFilter] = useState<string>('Journals');
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem('viaconnect-practitioner-active-sources');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('viaconnect-practitioner-active-sources', JSON.stringify(activeSources));
    } catch {}
  }, [activeSources]);
    ...
    >
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div
          ...
          style={{ background: 'linear-gradient(135deg, rgba(74,144,217,0.2), rgba(74,144,217,0.05))' }}
        >
          <Layers className="w-4 h-4 text-[#4A90D9]" />
          <span className="text-sm font-semibold text-[#4A90D9]">ViaConnect GeneX360</span>
        </div>
        <h1
          className="text-[42px] font-extrabold leading-tight bg-clip-text text-transparent mb-3"
          style={{ backgroundImage: 'linear-gradient(135deg, #4A90D9, #B75E18)' }}
        >
          Research Hub
        </h1>
        <p className="text-[17px] text-[#A0AEC0] max-w-[720px]">
          Access curated clinical research and evidence-based media sources to support patient care.
          Stay current with the latest genomic studies, pharmacogenomic findings, and translational medicine breakthroughs.
        </p>
      </div>
      ...
      <div className="max-w-7xl mx-auto mb-6">
        <div
          className="rounded-xl p-4 flex items-center gap-3 border border-[#4A90D9]/20"
          style={{ background: 'rgba(74,144,217,0.06)', backdropFilter: 'blur(12px)' }}
        >
          <Info className="w-5 h-5 text-[#4A90D9] flex-shrink-0" />
          <p className="text-sm text-[#A0AEC0]">
            <span className="text-[#4A90D9] font-semibold">Pro tip:</span> Recommend sources to your patients from their profile page to help them stay informed with trusted content.
          </p>
        </div>
      </div>
      ...
      {activeSources.length > 0 && (
        <div className="max-w-7xl mx-auto mb-6">
          <div
            className="rounded-xl p-5 flex items-start gap-4 border-l-4 border-[#4A90D9]"
          ...
          <button
            ...
            style={{
              background: isSaved
                ...
                : 'linear-gradient(135deg, #4A90D9, #3570B0)',
            }}
          >
        ...
        <div className="flex items-center gap-3 overflow-x-auto no-scrollbar -mx-4 px-4 md:mx-0 md:px-0 md:flex-wrap md:overflow-visible">
          ...
          <div className="relative ml-auto flex-shrink-0">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2 rounded-lg text-sm text-white placeholder-[#A0AEC0] border border-white/10 outline-none transition-all duration-200 focus:border-[#4A90D9]"
