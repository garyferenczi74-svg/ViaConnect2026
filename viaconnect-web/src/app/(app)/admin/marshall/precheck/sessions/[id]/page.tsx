...
export default async function AdminSessionDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data } = await supabase.from("precheck_sessions").select("*").eq("id", id).maybeSingle();
  if (!data) notFound();
  const { data: findings } = await supabase.from("precheck_findings").select("*").eq("session_id", id).order("round");
  ...
  return (
    <div className="min-h-screen bg-[#1A2744] px-4 md:px-8 py-6">
      <Link href="/admin/marshall/precheck/sessions" className="text-white/50 hover:text-white flex items-center gap-1 text-xs mb-4">
      ...
      <div className="bg-[#1E3054] rounded-xl border border-white/[0.08] p-4 space-y-1 text-xs text-white/60">
        <div><b className="text-white">Practitioner:</b> <span className="font-mono">{s.practitioner_id as string}</span></div>
        <div><b className="text-white">Status:</b> {s.status as string}</div>
        <div><b className="text-white">Source:</b> {s.source as string}</div>
        <div><b className="text-white">Hash:</b> <span className="font-mono break-all">{s.draft_hash_sha256 as string}</span></div>
        <div><b className="text-white">Rule registry:</b> {s.rule_registry_version as string}</div>
        <div><b className="text-white">Created:</b> {new Date(s.created_at as string).toLocaleString()}</div>
      </div>
