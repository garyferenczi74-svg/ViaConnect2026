/**
 * Prompt 227e: apply retraction watch schema (Bearer CRON_SECRET).
 */
import { isCronAuthorized } from '@/lib/jeffery/ops/cronAuth';
import { PROMPT_227E_MIGRATIONS } from '@/lib/kb/migrations/embedded227e';
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    try {
      for (const migration of PROMPT_227E_MIGRATIONS) {
        try {
          await sql.unsafe(migration.sql);
          results.push({ file: migration.file, ok: true });
        } catch (err) {
          const message = err instanceof Error ? err.message : String(err);
          safeLog.error('cron.apply-227e', 'migration failed', {
            file: migration.file,
            error: message,
          });
    ...
    const message = err instanceof Error ? err.message : String(err);
    safeLog.error('cron.apply-227e', 'threw', { error: message });
