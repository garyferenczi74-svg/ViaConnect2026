/**
 * Prompt 226d: apply Wave A schema (Bearer CRON_SECRET).
 * Uses embedded SQL. Never returns secrets.
 */
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    try {
      for (const migration of PROMPT_226D_MIGRATIONS) {
        try {
          await sql.unsafe(migration.sql);
          results.push({ file: migration.file, ok: true });
        } catch (err) {
          const message = err instanceof Error ? err.message : String(err);
          safeLog.error('cron.apply-226d', 'migration failed', {
            file: migration.file,
            error: message,
          });
    ...
    const message = err instanceof Error ? err.message : String(err);
    safeLog.error('cron.apply-226d', 'threw', { error: message });
