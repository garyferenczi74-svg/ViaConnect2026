/**
 * Prompt 227ah: apply Sherlock curation role migration (Bearer CRON_SECRET).
 */
import { isCronAuthorized } from '@/lib/jeffery/ops/cronAuth';
import { PROMPT_227AH_MIGRATIONS } from '@/lib/kb/migrations/embedded227ah';
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    try {
      for (const migration of PROMPT_227AH_MIGRATIONS) {
        try {
          await sql.unsafe(migration.sql);
          results.push({ file: migration.file, ok: true });
        } catch (err) {
          const message = err instanceof Error ? err.message : String(err);
          safeLog.error('cron.apply-227ah', 'migration failed', {
            file: migration.file,
            error: message,
          });
    ...
    const message = err instanceof Error ? err.message : String(err);
    safeLog.error('cron.apply-227ah', 'threw', { error: message });
