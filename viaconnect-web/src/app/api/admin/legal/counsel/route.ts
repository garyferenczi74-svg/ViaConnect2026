...
async function requireAdmin(supabase: ReturnType<typeof createClient>) {
  const { data: { user } } = await withTimeout(supabase.auth.getUser(), 5000, 'api.admin.legal.counsel.auth');
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.admin.legal.counsel', 'GET timeout', { error: err });
      return NextResponse.json({ error: 'Request timed out.' }, { status: 503 });
    }
    safeLog.error('api.admin.legal.counsel', 'unexpected error', { error: err });
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.admin.legal.counsel', 'POST timeout', { error: err });
      return NextResponse.json({ error: 'Request timed out.' }, { status: 503 });
    }
    safeLog.error('api.admin.legal.counsel', 'unexpected error', { error: err });
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
