...
async function requireLegalOps(supabase: ReturnType<typeof createClient>) {
  const { data: { user } } = await withTimeout(supabase.auth.getUser(), 5000, 'api.admin.legal.enforcement-actions.auth');
...
export async function GET(_request: NextRequest, props: { params: Promise<{ actionId: string }> }): Promise<NextResponse> {
  ...
  try {
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.admin.legal.enforcement-actions', 'GET timeout', { error: err });
      return NextResponse.json({ error: 'Request timed out.' }, { status: 503 });
    }
    safeLog.error('api.admin.legal.enforcement-actions', 'unexpected error', { error: err });
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

export async function PATCH(request: NextRequest, props: { params: Promise<{ actionId: string }> }): Promise<NextResponse> {
  const params = await props.params;
  try {
    const supabase = await createClient();
    const ctx = await requireLegalOps(supabase);
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.admin.legal.enforcement-actions', 'PATCH timeout', { error: err });
      return NextResponse.json({ error: 'Request timed out.' }, { status: 503 });
    }
    safeLog.error('api.admin.legal.enforcement-actions', 'unexpected error', { error: err });
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
