...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.admin.legal.customs.recordations', 'GET timeout', { error: err });
      return NextResponse.json({ error: 'Request timed out.' }, { status: 503 });
    }
    safeLog.error('api.admin.legal.customs.recordations', 'unexpected error', { error: err });
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.admin.legal.customs.recordations', 'POST timeout', { error: err });
      return NextResponse.json({ error: 'Request timed out.' }, { status: 503 });
    }
    safeLog.error('api.admin.legal.customs.recordations', 'unexpected error', { error: err });
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
