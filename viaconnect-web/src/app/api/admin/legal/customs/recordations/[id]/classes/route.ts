...
async function requireLegalOps(supabase: ReturnType<typeof createClient>) {
  const { data: { user } } = await withTimeout(supabase.auth.getUser(), 5000, 'api.admin.legal.customs.recordations.classes.auth');
  if (!user) {
    return {
      ok: false as const,
...
export async function POST(
  ...
  try {
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.admin.legal.customs.recordations.classes', 'POST timeout', { error: err });
      return NextResponse.json({ error: 'Request timed out.' }, { status: 503 });
    }
    safeLog.error('api.admin.legal.customs.recordations.classes', 'unexpected error', { error: err });
...
export async function DELETE(
  ...
  try {
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.admin.legal.customs.recordations.classes', 'DELETE timeout', { error: err });
      return NextResponse.json({ error: 'Request timed out.' }, { status: 503 });
    }
    safeLog.error('api.admin.legal.customs.recordations.classes', 'unexpected error', { error: err });
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
