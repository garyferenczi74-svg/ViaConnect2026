...
export async function POST(
  ...
  try {
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.admin.legal.customs.recordations.ceo-approve', 'POST timeout', { error: err });
      return NextResponse.json({ error: 'Request timed out.' }, { status: 503 });
    }
    safeLog.error('api.admin.legal.customs.recordations.ceo-approve', 'unexpected error', { error: err });
