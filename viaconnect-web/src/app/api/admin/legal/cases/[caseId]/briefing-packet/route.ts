...
export async function GET(
  ...
  try {
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.admin.legal.cases.briefing-packet', 'GET timeout', { error: err });
      return NextResponse.json({ error: 'Request timed out.' }, { status: 503 });
    }
    safeLog.error('api.admin.legal.cases.briefing-packet', 'unexpected error', { error: err });
