...
export async function GET(
  ...
  try {
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.admin.legal.cases.engagements', 'GET timeout', { error: err });
      return NextResponse.json({ error: 'Request timed out.' }, { status: 503 });
    }
    safeLog.error('api.admin.legal.cases.engagements', 'unexpected error', { error: err });
...
export async function POST(
  ...
  try {
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.admin.legal.cases.engagements', 'POST timeout', { error: err });
      return NextResponse.json({ error: 'Request timed out.' }, { status: 503 });
    }
    safeLog.error('api.admin.legal.cases.engagements', 'unexpected error', { error: err });
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
