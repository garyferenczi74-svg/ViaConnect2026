...
async function requireAdmin(supabase: ReturnType<typeof createClient>) {
  ...
  const profileRes = await withTimeout(
    ...
    5000,
    'api.white-label.reporting.load-profile',
  );
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (section === 'program') {
      const enrollmentsRes = await withTimeout(
        (async () => sb
          .from('white_label_enrollments')
          .select('id, status, qualifying_path'))(),
        10000,
        'api.white-label.reporting.program.enrollments',
      );
      ...
      const ordersRes = await withTimeout(
        ...
        10000,
        'api.white-label.reporting.program.orders',
      );
      ...
      const dataRes = await withTimeout(
        ...
        10000,
        'api.white-label.reporting.practitioner',
      );
      rows = (dataRes.data ?? []) as Array<Record<string, unknown>>;
      ...
      const dataRes = await withTimeout(
        (async () => sb
          ...
          .limit(500))(),
        10000,
        'api.white-label.reporting.compliance',
      );
      rows = (dataRes.data ?? []) as Array<Record<string, unknown>>;
      ...
      const dataRes = await withTimeout(
        (async () => sb
          ...
          .limit(500))(),
        10000,
        'api.white-label.reporting.operations',
      );
      rows = (dataRes.data ?? []) as Array<Record<string, unknown>>;
      ...
      const ordersRes = await withTimeout(
        ...
        10000,
        'api.white-label.reporting.financial',
      );
      rows = ((ordersRes.data ?? []) as Array<{
    ...
    if (isTimeoutError(err)) {
      safeLog.error('api.white-label.reporting', 'database timeout', { error: err });
      return NextResponse.json({ error: 'Database operation timed out. Please try again.' }, { status: 503 });
    }
    safeLog.error('api.white-label.reporting', 'unexpected error', { error: err });
