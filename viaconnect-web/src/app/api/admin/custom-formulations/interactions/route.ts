  ...
  try {
    ...
    const insertRes = await withTimeout(
      (async () => supabase
        ...
        .single())(),
      8000,
      'api.custom-formulations.interactions.insert',
    );
    ...
    return NextResponse.json({ ok: true, id: (insertRes.data as { id: string }).id });
  } catch (err) {
    if (isTimeoutError(err)) {
      safeLog.error('api.custom-formulations.interactions', 'database timeout', { error: err });
      return NextResponse.json({ error: 'Database operation timed out. Please try again.' }, { status: 503 });
    }
    safeLog.error('api.custom-formulations.interactions', 'unexpected error', { error: err });
