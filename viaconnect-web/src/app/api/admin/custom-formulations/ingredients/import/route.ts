...
export async function POST(request: NextRequest) {
  try {
    ...
    const insertRes = await withTimeout(
      ...
      15000,
      'api.custom-formulations.ingredients.import',
    );
    ...
    return NextResponse.json({
      ok: true,
      imported: insertRes.count ?? 0,
      invalid_count: parsed.invalid.length,
      invalid_samples: parsed.invalid.slice(0, 20),
    });
  } catch (err) {
    if (isTimeoutError(err)) {
      safeLog.error('api.custom-formulations.ingredients-import', 'database timeout', { error: err });
      return NextResponse.json({ error: 'Database operation timed out. Please try again.' }, { status: 503 });
    }
    safeLog.error('api.custom-formulations.ingredients-import', 'unexpected error', { error: err });
