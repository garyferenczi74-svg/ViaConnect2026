...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { data: { user } } = await withTimeout(
      ...
      5000,
      'api.admin.practitioner-referrals.adjustment.auth',
    );
    ...
    const { data: profile } = await withTimeout(
      ...
      8000,
      'api.admin.practitioner-referrals.adjustment.profile',
    );
    ...
    const { data: target } = await withTimeout(
      (async () =>
        sb.from('practitioners').select('id').eq('id', parsed.data.practitioner_id).maybeSingle())(),
      8000,
      'api.admin.practitioner-referrals.adjustment.target',
    );
    if (!target) return NextResponse.json({ error: 'Target practitioner not found' }, { status: 404 });

    const { data: balance } = await withTimeout(
      (async () =>
        sb
          ...
          .maybeSingle())(),
      8000,
      'api.admin.practitioner-referrals.adjustment.balance',
    );
    const runningBalance = (balance?.current_balance_cents ?? 0) + parsed.data.amount_cents;

    const { data: ledger, error } = await withTimeout(
      (async () =>
        sb
          ...
          .maybeSingle())(),
      8000,
      'api.admin.practitioner-referrals.adjustment.insert',
    );
    if (error || !ledger?.id) {
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.admin.practitioner-referrals.adjustment', 'request timed out', { error: err });
      return NextResponse.json({ error: 'Service temporarily unavailable' }, { status: 503 });
    }
    safeLog.error('api.admin.practitioner-referrals.adjustment', 'unexpected error', { error: err });
