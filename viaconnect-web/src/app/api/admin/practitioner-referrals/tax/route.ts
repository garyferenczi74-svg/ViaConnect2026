...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { data: { user } } = await withTimeout(
      ...
      5000,
      'api.admin.practitioner-referrals.tax.list.auth',
    );
    ...
    const { data: profile } = await withTimeout(
      ...
      8000,
      'api.admin.practitioner-referrals.tax.list.profile',
    );
    ...
    const { data, error } = await withTimeout(
      (async () => q.limit(500))(),
      10000,
      'api.admin.practitioner-referrals.tax.list.query',
    );
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.admin.practitioner-referrals.tax.list', 'request timed out', { error: err });
      return NextResponse.json({ error: 'Service temporarily unavailable' }, { status: 503 });
    }
    safeLog.error('api.admin.practitioner-referrals.tax.list', 'unexpected error', { error: err });
