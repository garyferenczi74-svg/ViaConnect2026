  ...
  try {
    ...
    const prevRes = await withTimeout(
      (async () => supabase
        ...
        .maybeSingle())(),
      8000,
      'api.governance.decision-rights.load-prev',
    );
    ...
    const updateRes = await withTimeout(
      (async () => supabase
        ...
        .eq('id', params.ruleId))(),
      8000,
      'api.governance.decision-rights.update',
    );
    if (updateRes.error) {
    ...
    await withTimeout(
      (async () => supabase.from('governance_configuration_log').insert(logRow as never))(),
      8000,
      'api.governance.decision-rights.insert-log',
    );

    return NextResponse.json({ ok: true, fields_changed: Object.keys(diff.next).length });
  } catch (err) {
    if (isTimeoutError(err)) {
      safeLog.error('api.governance.decision-rights', 'database timeout', { ruleId: params.ruleId, error: err });
      return NextResponse.json({ error: 'Database operation timed out. Please try again.' }, { status: 503 });
    }
    safeLog.error('api.governance.decision-rights', 'unexpected error', { ruleId: params.ruleId, error: err });
