...
> {
  ...
  const profileRes = await withTimeout(
    ...
    5000,
    'api.soc2.auditor-grants.load-profile',
  );
...
export async function GET(_req: NextRequest) {
  try {
    ...
    if (isTimeoutError(err)) {
      safeLog.error('api.soc2.auditor-grants.list', 'database timeout', { error: err });
      return NextResponse.json({ error: 'Database operation timed out. Please try again.' }, { status: 503 });
    }
    safeLog.error('api.soc2.auditor-grants.list', 'unexpected error', { error: err });
  ...
  try {
    ...
    const { data, error } = await withTimeout(
      (async () => sb
        ...
        .single())(),
      8000,
      'api.soc2.auditor-grants.create',
    );
    if (error) {
      safeLog.error('api.soc2.auditor-grants.create', 'insert failed', { message: error.message });
    ...
    if (isTimeoutError(err)) {
      safeLog.error('api.soc2.auditor-grants.create', 'database timeout', { error: err });
      return NextResponse.json({ error: 'Database operation timed out. Please try again.' }, { status: 503 });
    }
    safeLog.error('api.soc2.auditor-grants.create', 'unexpected error', { error: err });
    return NextResponse.json({ error: 'An unexpected error occurred.' }, { status: 500 });
  }
}
