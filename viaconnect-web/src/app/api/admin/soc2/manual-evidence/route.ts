...
> {
  ...
  const profileRes = await withTimeout(
    (async () => supabase
      ...
      .maybeSingle())(),
    5000,
    'api.soc2.manual-evidence.load-profile',
  );
...
export async function GET(_req: NextRequest) {
  ...
  const rows: ManualEvidenceRow[] = ((data ?? []) as DbRow[]).map((r) => ({
    ...
    if (isTimeoutError(err)) {
      safeLog.error('api.soc2.manual-evidence.list', 'database timeout', { error: err });
      return NextResponse.json({ error: 'Database operation timed out. Please try again.' }, { status: 503 });
    }
    safeLog.error('api.soc2.manual-evidence.list', 'unexpected error', { error: err });
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      if (innerErr instanceof ManualEvidenceUploadError) {
        return NextResponse.json({ error: innerErr.code, message: innerErr.message }, { status: 400 });
      }
      throw innerErr;
    }
  } catch (err) {
    if (isTimeoutError(err)) {
      safeLog.error('api.soc2.manual-evidence.upload', 'database timeout', { error: err });
      return NextResponse.json({ error: 'Database operation timed out. Please try again.' }, { status: 503 });
    }
    safeLog.error('api.soc2.manual-evidence.upload', 'unexpected error', { error: err });
