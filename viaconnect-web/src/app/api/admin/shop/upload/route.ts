...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const uploadRes = await withTimeout(
      ...
      15000,
      'api.admin.shop.upload.storage',
    );
    const upErr = (uploadRes as { error: { message?: string } | null }).error;
    if (upErr) {
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.admin.shop.upload', 'request timed out', { error: err });
      return NextResponse.json({ error: 'Service temporarily unavailable' }, { status: 503 });
    }
    safeLog.error('api.admin.shop.upload', 'unexpected error', { error: err });
