  ...
  try {
    ...
    const { data: events } = await withTimeout(
      ...
      (async () => (supabase as any)
        ...
        .limit(50))(),
      8000,
      'api.marketing.variants.events',
    );

    return NextResponse.json({ variant, events: events ?? [] });
  } catch (err) {
    if (isTimeoutError(err)) {
      safeLog.warn('api.marketing.variants', 'get timeout', { error: err });
  ...
  try {
    ...
    const { data: existing } = await withTimeout(
      ...
      (async () => (supabase as any)
        ...
        .maybeSingle())(),
      8000,
      'api.marketing.variants.preupdate',
    );
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.marketing.variants', 'update timeout', { error: err });
      return NextResponse.json({ error: 'Operation timed out.' }, { status: 503 });
    }
    safeLog.error('api.marketing.variants', 'unexpected error', { error: err });
    return NextResponse.json({ error: 'An unexpected error occurred.' }, { status: 500 });
