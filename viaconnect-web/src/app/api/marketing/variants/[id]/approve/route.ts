  ...
  try {
    ...
    const { data: variant } = await withTimeout(
      ...
      (async () => (supabase as any)
        ...
        .maybeSingle())(),
      8000,
      'api.marketing.variants.approve.read',
    );
    ...
    const { error } = await withTimeout(
      ...
      (async () => (supabase as any)
        ...
        .eq('id', params.id))(),
      8000,
      'api.marketing.variants.approve.update',
    );
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.marketing.variants.approve', 'timeout', { error: err });
      return NextResponse.json({ error: 'Operation timed out.' }, { status: 503 });
    }
    safeLog.error('api.marketing.variants.approve', 'unexpected error', { error: err });
