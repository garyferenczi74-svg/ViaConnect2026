...
export async function POST(request: Request) {
  ...
  try {
    ...
    const rationale = patterns.length > 0
      ? `Based on your health assessment, Ultrathink detected ${patterns.length} pattern${patterns.length > 1 ? 's' : ''}: ${patternNames}. Your personalized ViaConnect protocol addresses these with Maximum Bioavailability formulations targeting your specific needs.`
      : `Based on your ${context.goals.length} health goals and lifestyle profile, your personalized ViaConnect protocol delivers Maximum Bioavailability formulations targeting your specific needs.`;
