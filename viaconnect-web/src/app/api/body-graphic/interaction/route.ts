...
export async function POST(request: Request) {
  try {
    ...
    const { error } = await withTimeout(
      ...
      (async () => (sb as any).from("body_graphic_interactions").insert({
        user_id: user.id,
      ...
      8000,
      'api.body-graphic.interaction.insert',
    );
    if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
    return NextResponse.json({ ok: true });
  } catch (err) {
    if (isTimeoutError(err)) {
      safeLog.warn('api.body-graphic.interaction', 'timeout', { error: err });
      return NextResponse.json({ ok: false, error: 'timeout' }, { status: 503 });
    }
    safeLog.error('api.body-graphic.interaction', 'unexpected error', { error: err });
