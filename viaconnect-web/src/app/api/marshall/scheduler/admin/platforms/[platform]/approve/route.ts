...
export async function POST(req: NextRequest, { params }: { params: { platform: string } }) {
  try {
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.marshall.scheduler.admin.approve', 'request timeout', { error: err });
      return NextResponse.json({ error: 'timeout' }, { status: 503 });
    }
    safeLog.error('api.marshall.scheduler.admin.approve', 'unexpected error', { error: err });
