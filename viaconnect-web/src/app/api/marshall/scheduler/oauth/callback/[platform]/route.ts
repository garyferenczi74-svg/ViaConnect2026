...
export async function GET(req: NextRequest, { params }: { params: { platform: string } }) {
  try {
    ...
    const { data: { user } } = await withTimeout(session.auth.getUser(), 5000, 'api.marshall.scheduler.oauth.callback.auth');
    ...
    try {
      ...
      safeLog.error('api.marshall.scheduler.oauth.callback', 'complete failed', { platform, error: err });
      const redirectUrl = new URL(PORTAL_PATH, url);
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.marshall.scheduler.oauth.callback', 'request timeout', { error: err });
      return NextResponse.json({ error: 'timeout' }, { status: 503 });
    }
    safeLog.error('api.marshall.scheduler.oauth.callback', 'unexpected error', { error: err });
