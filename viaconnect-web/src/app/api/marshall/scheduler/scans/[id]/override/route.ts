...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.marshall.scheduler.scans.override', 'request timeout', { error: err });
      return NextResponse.json({ error: 'timeout' }, { status: 503 });
    }
    safeLog.error('api.marshall.scheduler.scans.override', 'unexpected error', { error: err });
