...
export async function POST(req: NextRequest) {
  ...
  try {
    const outcome = await withTimeout(
      ...
      15000,
      'api.marshall.scheduler.webhooks.sprout',
    );
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.marshall.scheduler.webhooks.sprout', 'webhook handler timeout', { error: err });
      return NextResponse.json({ error: 'timeout' }, { status: 504 });
    }
    schedulerLogger.error('[webhook/sprout] persist error', { error: (err as Error).message });
    safeLog.error('api.marshall.scheduler.webhooks.sprout', 'persist failed', { error: err });
