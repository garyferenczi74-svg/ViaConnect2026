...
export async function GET(_req: NextRequest) {
  try {
    ...
    const { data: { user } } = await withTimeout(supabase.auth.getUser(), 5000, 'api.marshall.vision.review.queue.auth');
    if (!user) {
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.marshall.vision.review.queue', 'request timeout', { error: err });
      return NextResponse.json({ error: 'timeout' }, { status: 503 });
    }
    safeLog.error('api.marshall.vision.review.queue', 'unexpected error', { error: err });
