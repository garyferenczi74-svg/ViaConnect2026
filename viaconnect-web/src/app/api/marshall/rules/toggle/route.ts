...
export async function POST(req: Request) {
  try {
    ...
    if (typeof ruleId !== "string" || typeof enabledRaw !== "string") {
      return NextResponse.json({ error: "ruleId and enabled required" }, { status: 400 });
    }
    const enabled = enabledRaw === "true";

    const svc = serviceClient();
    const { error } = await svc.from("compliance_rules").update({ enabled, reviewed_by: user.id }).eq("id", ruleId);
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });

    await svc.from("compliance_audit_log").insert({
      event_type: `rule.${enabled ? "enabled" : "disabled"}`,
      actor_type: "user",
      actor_id: user.id,
    ...
    if (isTimeoutError(err)) {
      safeLog.warn('api.marshall.rules.toggle', 'request timeout', { error: err });
      return NextResponse.json({ error: 'timeout' }, { status: 503 });
    }
    safeLog.error('api.marshall.rules.toggle', 'unexpected error', { error: err });
