  ...
  try {
    ...
    try {
      const authResult = await withTimeout(
        ...
        5000,
        'api.white-label.checkout-final.auth',
      );
      user = authResult.data.user;
    } catch (err) {
      if (isTimeoutError(err)) {
        safeLog.error('api.white-label.checkout-final', 'auth timeout', { requestId, orderId: params.orderId, error: err });
    ...
    const practitionerRes = await withTimeout(
      ...
      8000,
      'api.white-label.checkout-final.practitioner-load',
    );
    ...
    const orderRes = await withTimeout(
      (async () => sb
        ...
        .maybeSingle())(),
      8000,
      'api.white-label.checkout-final.order-load',
    );
    ...
    try {
      ...
      safeLog.info('api.white-label.checkout-final', 'payment intent created', { requestId, orderId: params.orderId, userId: user.id });
      return NextResponse.json(result);
    } catch (e) {
      if (isCircuitBreakerError(e)) {
        safeLog.warn('api.white-label.checkout-final', 'stripe circuit open', { requestId, orderId: params.orderId, error: e });
        return NextResponse.json({ error: 'Payment service temporarily unavailable. Please retry shortly.' }, { status: 503 });
      }
      if (isTimeoutError(e)) {
        safeLog.error('api.white-label.checkout-final', 'payment intent timeout', { requestId, orderId: params.orderId, error: e });
        return NextResponse.json({ error: 'Payment setup took too long. Please try again.' }, { status: 504 });
      }
      safeLog.error('api.white-label.checkout-final', 'payment intent failed', { requestId, orderId: params.orderId, error: e });
      return NextResponse.json({ error: (e as Error).message }, { status: 400 });
    }
  } catch (err) {
    if (isTimeoutError(err)) {
      safeLog.error('api.white-label.checkout-final', 'database timeout', { requestId, orderId: params.orderId, error: err });
      return NextResponse.json({ error: 'Database operation timed out. Please try again.' }, { status: 503 });
    }
    safeLog.error('api.white-label.checkout-final', 'unexpected error', { requestId, orderId: params.orderId, error: err });
