// Prompt 212: POST /api/integrations/health-sync
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.source === "health_connect" && process.env.HEALTH_CONNECT_ENABLED !== "1") {
      // Clean capability flag: Android Health Connect not fully shipped.
    ...
    // Process immediately for small batches (still bounded)
    if (samples.length <= 500) {
      void processPendingEvents(admin, 5).catch((err) =>
