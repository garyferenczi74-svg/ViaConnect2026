...
export async function GET(req: NextRequest) {
  try {
    ...
    const [
      ...
      withTimeout(
        admin
          ...
          // hrv_ms / resting_hr_bpm + source_app needed for Task 7b's
          // hrv/resting_hr contributor rows. limit raised 10 -> 30 (caveat
          // #2) since Apple Health XML can write multiple per-day rows
          // (one per metricKey) that could otherwise starve Whoop/Oura out
          // of a small window.
          .select('source_provider, recovery_score, cycle_date, hrv_ms, resting_hr_bpm, source_app')
          .eq('user_id', userId)
          .is('deleted_at', null)
      ...
      withTimeout(
        admin
          ...
          .select('source_provider, steps, metric_date, source_app')
          .eq('user_id', userId)
          .is('deleted_at', null)
    ...
    const dailyVitalsRows = (dailyVitalsRes.data ?? []) as DailyVitalsIngestRow[];
