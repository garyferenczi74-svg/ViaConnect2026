 ...
 * migrations dir with controlled mtimes) under a fixed injected clock
 * (--now-ms), asserting:
 *   1. a post-baseline unmanifested migration older than grace exits 1
 *      and is named
 *   2. the same migration younger than grace exits 0 (grace window)
 ...
 *   5. name-contains-stem matching holds beyond grace, including the
...
function daysBeforeNow(days: number): number {
...
function createFixture(entries: ManifestEntry[], files: FixtureFile[]): string {
  ...
  for (const file of files) {
    const filePath = join(dir, "migrations", file.fileName);
    writeFileSync(filePath, "select 1;\n");
    const stamp = new Date(file.mtimeMs);
...
describe("check-migration-parity CLI", () => {
  ...
  it("exits 0 while the unmanifested migration is younger than grace", () => {
    ...
    expect(result.stdout).toContain(
      "[grace] 20260708090000_prompt_fixture_unsigned.sql",
    );
  });
  ...
  it("exits 0 for pre-baseline breaks on both sides (accepted history)", () => {
    const dir = createFixture(
      ...
      [
        // Pre-baseline repo file never recorded as applied, long past grace.
        {
          fileName: "20260101000000_legacy_never_applied.sql",
          mtimeMs: daysBeforeNow(120),
        },
      ],
    );
    const result = runChecker(dir);
  ...
  it("matches by name-contains-stem beyond grace, including double-apply and CLI-style rows", () => {
    const dir = createFixture(
      [
