...
**Next-order (additive):** When `PROTOCOL_NEXT_ORDER_ENABLED` is true, attach `protocol_entries` from protocol SSOT + `CAQ_REPLACEMENT_MAP` (exact pairs then category). Competitors stay `current`. Unmapped (greens/all-in-one) → no fake SKU. Not Hannah RAG invent. Flag default **false**. GeneX360 Soft = engines SSOT + catalog map; GeneXM naming; flag `GENEX360_NEXT_ORDER_ENABLED` default **false**. Labs Soft = engines/labs SSOT + on-file map only (honesty empty if zero pairs); not RAG invent; not treat-this-lab; flag `LABS_NEXT_ORDER_ENABLED` default **false**.
...
**CAQ-only lock:** MediSearch / Prove It / Suppie feeds stay on CAQ compare — not grounded Sources / Hannah RAG. Flag `CAQ_MEDISEARCH_ENABLED` default **false**.

**Chat path (Stage A wrap):** `get_education` is wired in-process on grounded chat (READ consumer `peptide_education_entries` + Lex/FAQ `safety_never_say` fixtures; first RAG slice ≤20 allowlisted ids; no HTTP loopback). Allowlist retriever chunks append cite_id + label into Sources on education success only (deduped vs tool PMIDs/sources; empty → no phantom cites). Research Hub folds approved+active `authorities_sources` as cite-only `auth:…` Sources (≤15; stored label/domain only) plus optional Hounddog URL cites (≤10; title/host only; score≥50 non-GLP; default OFF). ViaCura supplement education is a separate ≤5 cite lane (Elizabeth first-slice `cite_id`+label only; cite≠dose; OFF carve is allowlist-exact; competitor SKU seeds stay CAQ comparison only and are not Hannah Sources). Combined edu+safety+ViaCura+authorities attach still ≤35 — past-cap expand needs a **NEW GATE**. Flag `LLM_GROUNDED_CHAT_ENABLED` stays default **false**. Production empty/unmappable/failed approved-authority READ may attach only the existing gated static seed (`cite_id`+label; not loaded evidence). The chat injection seam `loadAuthorityCites: async () => []` remains zero cites / no phantom Sources. Do not treat those seams as the same. Elizabeth corpus-feed is seed/docs only — no Elizabeth UI; HannahAI remains member-facing.
