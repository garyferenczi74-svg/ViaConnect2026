...
| Tool | Engine |
| --- | --- |
| `get_protocol` | Protocol engine / stored protocol. Next-order = protocol + CAQ map (not Hannah RAG invent); flag `PROTOCOL_NEXT_ORDER_ENABLED` default false. GeneX360 Soft = engines SSOT + catalog map; GeneXM naming; flag `GENEX360_NEXT_ORDER_ENABLED` default false |
...
- Next-order suggestions = daily protocol + CAQ replacement map — not Hannah RAG invent. Flag `PROTOCOL_NEXT_ORDER_ENABLED` default false. GeneX360 Soft = engines SSOT + catalog map; GeneXM naming; flag `GENEX360_NEXT_ORDER_ENABLED` default false.
