...
- ViaCura supplement education is a **separate** ≤5 allowlist lane (`STAGE_A_VIACURA_SUPPLEMENT_EDU_ALLOWLIST`; Elizabeth-approved first slice: `edu-viacura:nad-plus` / `mthfr-plus` / `catalog-index` / `rise-plus` / `relax-plus`). Surgical OFF-list carve for those `cite_id`s + stored labels only — keep blanket `via-?cura`. PeptideIQ edu+safety ≤20 unchanged. Attach order on education success: PeptideIQ edu cites → ViaCura supplement edu → authorities → optional Hounddog; combined extra cites still ≤35 (overflow dropped). Elizabeth curates allowlisted consumer labels into the seed; HannahAI remains the member-facing voice; no Elizabeth UI. `cite_id`+label only; not dose SSOT. Competitor catalog/SKU data is **CAQ comparison only** — not Hannah's store; do not wire into this lane. Flag off.
...
- Next-order suggestions = daily protocol + CAQ replacement map — not Hannah RAG invent. Flag `PROTOCOL_NEXT_ORDER_ENABLED` default false. GeneX360 Soft = engines SSOT + catalog map; GeneXM naming; flag `GENEX360_NEXT_ORDER_ENABLED` default false. Labs Soft = engines/labs SSOT + on-file map only (honesty empty if zero pairs); not RAG invent; not treat-this-lab; flag `LABS_NEXT_ORDER_ENABLED` default false.
